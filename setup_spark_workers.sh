...
sudo -H -u rammy bash -c 'source $HOME/.bashrc && spark-class org.apache.spark.deploy.worker.Worker --webui-port 8080 spark://192.168.1.1:7077 >> /opt/software/spark-worker.out &' &
